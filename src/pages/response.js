...
export default function Response({ navigation, route }) {
  ...
  function goToParent() {
    navigation.navigate(post.parentIsPost ? "Post" : "Response", {
      id: post.parent,
    });
  }
  ...
  async function loadPost() {
    const { id } = route.params;
    const newPost = await dbReadDoc("responses", id);
    setParent(
      await dbReadDoc(
        newPost.parentIsPost ? "posts" : "responses",
        newPost.parent
      )
    );
    setPost(newPost);
        ...
        {parent ? (
          ...
          >
            ...
            >
              <Pressable
                onPress={() => {
                  if (parent.writer === auth.currentUser.uid) goToProfile();
                  else goToUser(parent.writer);
                }}
              >
                <Image
                  style={{
                  ...
                  source={usersObj[parent.writer].icon}
                />
              </Pressable>
              <View
                style={{
              ...
              >
                ...
                >
                  <Text>{parent.text.slice(0, 180)}</Text>
                </Pressable>
              </View>
              <View style={{ gap: 10 }}>
                {parent.likes.some((user) => user === auth.currentUser.uid) ? (
                  <Pressable
                    onPress={() => {
                      dbUpdate("posts", parent.id, {
                        likes: parent.likes.filter(
                  ...
                  <Pressable
                    onPress={() => {
                      dbUpdate("posts", parent.id, {
                        likes: [...parent.likes, auth.currentUser.uid],
                      }).then(loadPost);
                    }}
                  >
                    <Entypo
                ...
                {parent.writer === auth.currentUser.uid ? (
                  <Pressable
                    onPress={() =>
                      dbDelete("parents", parent.id).then(() => {
                        loadPost();
                      })
                    }
                  >
              ...
              <Text style={{ width: 60 }}>{formatTime(parent)}</Text>
            </View>
            {parent.likes.length > 0 ? (
              <View style={{ flexDirection: "row", gap: 5 }}>
                <Text>Curtido por</Text>
                {parent.likes.map((userId) => {
          ...
          >
            ...
            >
              ...
              >
                ...
                <Pressable
                  onPress={() => {
                    console.log("Post Page to be implemented");
                  }}
                >
